...
| React dashboard | agent output을 chat이 아니라 observability surface로 보는 관점 | [Article 05](05-react-agent-trace-dashboard.ko.md) |
| Failure-mode evals | happy path보다 failure mode를 검증하려는 태도 | [Article 06](06-agent-failure-modes-and-evals.ko.md) |
...
- Failure-mode eval runner: implemented with deterministic scenarios
...
4. CLI는 현재 실행 결과와 판단 과정을 terminal에서 보여주고, dashboard는 같은 trace를 observability surface로 보여준다.
